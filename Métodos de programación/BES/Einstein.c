...
int main () {
	...
	//Se definen variables que ayudaran a medir el tiempo que demora en ejecutar el programa
	time_t tTotalIni, tTotalFin, tIniCrearCS, tFinCrearCS, tIniFiltrarCS, tFinFiltrarCS;

	tTotalIni = time(NULL);

	//A continuacion se definen los pasos para resolver el acertijo de Einstein mediante BES

	//Se define la forma general de la solucion como un arreglo char de largo 25
	//Debido a que son muchas posible soluciones, estas se escriben en un archivo.

	//Se inicia el contador para medir el tiempo que demora el programa en generar todas las soluciones
	tIniCrearCS = time(NULL);

	crearConjuntoSolucionEstatico();

	tFinCrearCS = time(NULL);

	



	//Se procede a verificar las restricciones para cada una de las soluciones
	int cantidadSoluciones = 0;

	tIniFiltrarCS = time(NULL);

	char** soluciones = filtrarConjuntoSolucion(&cantidadSoluciones);

	tFinFiltrarCS = time(NULL);

	printf("La cantidad de soluciones posibles es: %d\n", cantidadSoluciones);

	printf("A continuacion se presenta la o las soluciones obtenidas:\n");

	for (int i = 0; i < cantidadSoluciones; ++i)
	{
		imprimirSolucion(soluciones[i], i+1);
	}
	...
	printf("\nA continuacion se presentan los tiempos del programa: \n");
	printf("\nTiempo crear permutaciones y variaciones: %f\n",difftime(tFinCrearCS,tIniCrearCS));

	printf("\nTiempo en verificar soluciones: %f\n",difftime(tFinFiltrarCS,tIniFiltrarCS));

	printf("\nTiempo total del programa: %f\n",difftime(tTotalFin,tTotalIni));
